...
def index():
    ...
    top_7movies = dataset_movie.sort_values(by=['Movie_Votes'], ascending=False).head(7)
    top_7movies = top_7movies.set_index('Title_Movie')
    ax = top_7movies['Movie_Votes'].sort_values(ascending=True).plot(kind='barh', title='Top 7 Movies')
